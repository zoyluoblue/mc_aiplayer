...
- 🧠 **大模型负责理解意图。** 你说"挖 3 颗钻石",DeepSeek 理解它,并从 **56 个工具**里自主选择。
- ⚙️ **确定性引擎负责可靠执行。** 倒推规划器把目标拆成依赖正确的完整计划,**44 个自包含任务状态机**稳稳执行,**五层安全网**保证 bot 不轻易送命。
...
| | |
|---|---|
| 🗣️ **自然语言驱动** | 中文 / 英文直接下命令,DeepSeek 大模型 + 56 工具理解并执行。 |
| 🎯 **目标倒推规划** | 一句目标 → 依赖正确的多步计划,无需手动拆解。 |
| 🧩 **LLM + 确定性混合** | 模型负责推理,引擎负责执行。既灵活又可靠。 |
| 🛡️ **五层安全网** | 溺水、岩浆、窒息、卡死、威胁、困死——每 tick 自检自救。 |
| 🧍 **拟人化行为** | 台阶式斜向下挖(绝不直挖脚下)、不瞬移、不兔子跳。 |
| ⛏️ **完整生存闭环** | 挖矿、熔炼、合成、战斗、狩猎、种田、养殖、建造、钓鱼、交易、睡觉。 |
...
<p align="center"><sub><b>151</b> 个类 · <b>23.4K</b> 行 · <b>56</b> 工具 · <b>44</b> 任务状态机 · <b>5</b> 层安全网</sub></p>
...
- [ ] 更丰富的客户端交互
- [ ] 长期记忆恢复
- [ ] 自动化验证场景(`/aibot verify`、`/aibot deplint`)
